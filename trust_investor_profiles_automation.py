...
cases = json.loads(open('cases/create_trust_investor_profile.json', 'r').read())
...
def trust_investor_profiles_automation():
    testCases = []
    # Login to InstaCate
    create_trust_investor_profiles_cases_keys = cases["CREATE_INVESTOR_TRUST_PROFILES"].keys()
    testCases.append(',Create Trust Investor Test Cases, \n')
    for case in create_trust_investor_profiles_cases_keys:
        ...
        create_trust_investor_profiles_test = create_trust_investor_profiles(driver, case) # Initiating the Test Case
        driver.close() # Closing the window for fresh test case everytime

        print(create_trust_investor_profiles_test)
        testCases.append(create_trust_investor_profiles_test) # Appending the test case to the list
    return testCases
