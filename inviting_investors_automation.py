...
cases = json.loads(open('cases/invite_investors.json', 'r').read())
...
def inviting_investors_automation():
    testCases = []
    # Login to InstaCate
    invite_investors_cases_keys = cases["INVITE_INVESTORS"].keys()
    testCases.append(',Investor Invitation Test Cases, \n')
    for case in invite_investors_cases_keys:
        # Initializing and starting the driver
        driver = init_driver(driver_path)
        driver.get(base_url)
        driver.maximize_window()
        org_login(driver, "CASE5")
        invite_investors_test = invite_investors(driver, case) # Initiating the Test Case
        driver.close() # Closing the window for fresh test case everytime

        print(invite_investors_test)
        testCases.append(invite_investors_test) # Appending the test case to the list
    return testCases
