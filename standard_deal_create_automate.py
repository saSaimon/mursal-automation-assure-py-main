...
json_keys = json.loads(open('cases/standard_deal_create_cases.json', 'r').read())
...
def standard_deal_create(
    ...
    try:
        ...
        description = json_keys['STANDARD_DEAL_CREATE_CASES'][current_case]['DESCRIPTION']
        ...
        select_labs_deal = driver.find_element(By.ID, constants["KEYS"]["IDs"]["SELECT_STANDARD_DEAL"])
        ...
        deal_name = json_keys["STANDARD_DEAL_CREATE_CASES"][current_case]["DEAL_NAME"]
        deal_name_input = driver.find_element(By.NAME, "name")
        deal_name_input.send_keys(deal_name, unique_number)
        ...
        for each_p in all_Ps:
            if each_p.text == "Investor Must Be A Qualified Purchaser":
                each_p.click()
                break
        ...
        # check_qualified_purchaser = driver.find_element(By.XPATH, "/html/body/div[1]/div/div[2]/div[3]/form/div[4]/label/span[1]")
        # check_qualified_purchaser.click()

        target_amount = driver.find_element(By.NAME, "targetRaiseAmount")
        target_amount.send_keys("5000000")
